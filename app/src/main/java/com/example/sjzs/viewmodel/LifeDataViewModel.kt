...
import com.example.sjzs.model.paging.LifeListRemoteMediator
...
class LifeDataViewModel(application: Application) : AndroidViewModel(application) {
    //数据库
    val database = ListBeanDatabase.getDatabase(getApplication())
    val lifeLitDao = database.lifeListDao()
    ...
    val listData = Pager(
        config = config,
       remoteMediator = LifeListRemoteMediator("life","life", database, apiService)
    )
    {
        lifeLitDao.queryAllListBeans()
    }
        .flow
