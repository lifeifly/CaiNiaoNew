...
import com.example.sjzs.model.paging.TechListRemoteMediator
import com.example.sjzs.model.paging.WorldListRemoteMediator
import com.example.sjzs.model.room.ListBeanDatabase


class TechDataViewModel(application: Application) : AndroidViewModel(application) {
    //数据库
    val database = ListBeanDatabase.getDatabase(getApplication())
    val techListDao = database.techListDao()
    ...
    val listData = Pager(
        config = config,
       remoteMediator = TechListRemoteMediator("tech","tech", database, apiService)
    )
    {
        techListDao.queryAllListBeans()
    }
        .flow
