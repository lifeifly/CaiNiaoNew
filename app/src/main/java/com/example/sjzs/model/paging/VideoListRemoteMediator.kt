...
class VideoListRemoteMediator(
...
) : RemoteMediator<Int,VideoList >() {
    private val listBeanDao = database.videoListDao()
    ...
    override suspend fun load(
        loadType: LoadType,
        state: PagingState<Int, VideoList>
    ): MediatorResult {
        return try {
            ...
            val response = apiService.getVideoList("${loadKey}","100")
            Log.d("REFRESH", "load:4 "+response.list.size)
            database.withTransaction {
            ...
            MediatorResult.Success(
                endOfPaginationReached = loadKey + 1 == 10
            )
        } catch (e: IOException) {
