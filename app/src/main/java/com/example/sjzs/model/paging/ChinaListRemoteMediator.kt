...
class ChinaListRemoteMediator(
    private val query: String,
    private val category: String,
    private val database: ListBeanDatabase,
    private val apiService: ApiService
) : RemoteMediator<Int, ChinaList>() {
    private val listBeanDao = database.chinaListDao()
    ...
    override suspend fun load(
        loadType: LoadType,
        state: PagingState<Int, ChinaList>
    ): MediatorResult {
        return try {
            ...
            val response = apiService.getChinaData(category, "${loadKey}")
            Log.d("REFRESH", "loadkey:LIST 4"+response)
            database.withTransaction {
            ...
            Log.d("REFRESH1", "loadkey:LIST 1")
            MediatorResult.Error(e)
        } catch (e: HttpException) {
            Log.d("REFRESH2", "loadkey:LIST 1")
            MediatorResult.Error(e)
        }catch (e:NullPointerException){
            Log.d("REFRESH2", "loadkey:LIST 3")
            MediatorResult.Error(e)
        }
    }
