...
class LawPagingAdapter(
    private val context: Context
) : BaseBindingAdapter<LawList,
    RecyclerView.ViewHolder>(context),
    IItemClick<LawList> {

    override fun onCreateVH(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return LawRvItemViewHolder.instance(parent)
    }

    override fun onBindVH(holder: RecyclerView.ViewHolder, position: Int) {
        val lawList = getItem(position) ?: return
        (holder as LawRvItemViewHolder).bindTo(lawList, this)
    }

    /**
     * 处理点击跳转的逻辑
     */
    override fun onClick(view: View, t: LawList) {
        val intent = Intent()
