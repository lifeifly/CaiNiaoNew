...
import com.example.sjzs.model.bean.EntertainmentList
...
class EnterPagingAdapter(
    private val context: Context
) : BaseBindingAdapter<EntertainmentList,
        RecyclerView.ViewHolder>(context),
    IItemClick<EntertainmentList> {

    override fun onCreateVH(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return EnterRvItemViewHolder.instance(parent)
    }

    override fun onBindVH(holder: RecyclerView.ViewHolder, position: Int) {
        val entertainmentList = getItem(position) ?: return
        (holder as EnterRvItemViewHolder).bindTo(entertainmentList, this)
    }

    /**
     * 处理点击跳转的逻辑
     */
    override fun onClick(view: View, t: EntertainmentList) {
        val intent = Intent()
